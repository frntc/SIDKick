...
#define SUB_SAMPLE_PRE  1


//extern const unsigned short resonance[16][1 << 16];
...
protected:
  ...
  typedef struct {
    ...
    unsigned short gain[16][(1 << 16)/SUB_SAMPLE_PRE];
    unsigned short mixer[(mixer_offset<8>::value)/SUB_SAMPLE_PRE];
...
void Filter::clock(int voice1, int voice2, int voice3)
{
  ...
  if (sid_model == 0) {
    ...
    Vhp = f.summer[(offset + f.gain[_8_div_Q][Vbp/SUB_SAMPLE_PRE] + Vlp + Vi)/SUB_SAMPLE_PRE];
  }
  else {
    // MOS 8580.
...
for my $mix (0..2**@i-1) {
  ...
  */
  ...
  return (short)(f.gain[vol][f.mixer[(offset + Vi)/SUB_SAMPLE_PRE]/SUB_SAMPLE_PRE] - (1 << 15));
}
...
int Filter::solve_integrate_6581(int dt, int vi, int& vx, int& vc, model_filter_t& mf)
{
  ...
  int kVg = vcr_kVg[((Vddt_Vw_2 + (Vgdt_2 >> 1)) >> 16)/SUB_SAMPLE_PRE];
  ...
  int n_I_vcr = int(unsigned(vcr_n_Ids_term[Vgs/SUB_SAMPLE_PRE] - vcr_n_Ids_term[Vgd/SUB_SAMPLE_PRE]) << 15);
